...
def Origination_post():
    ...
    if length<1:
        ...
        return render_template('NoSBCError.html')
    result = curlInitiator(sbcIp,zone,zoneId,sipSigPort,ipv4,sipTrunk,media,directMedia,nature)

    if result == "List mismatch":
        flash("ERROR!!! the count of Sig IP does not match to the count of selected SBC")
        return render_template('ListUnequal.html')
...
def PrivateTerm_post():
    ...
    if result == "Error sbc list and ipv4 list":
        flash("ERROR!!!count of selected SBC is not equal to the count of SIP sig IP address")
        return render_template('ListUnequal.html')
